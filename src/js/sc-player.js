...
define(['soundmanager2'],function(SoundManager){
	...
	*/
	...
	var SoundCloudPlayer = function(tracks, config){
		...
		, sc_resolve_url = "//api.soundcloud.com/resolve?url=https://soundcloud.com"
		...
		_this.resolve_track = function(url, cb){
			...
			promise.done(function(_track){
				if( _track.tracks && _track.tracks.length > 0 ){
					...
					_track = tracks[0];
				}else if (_track.length>0 && _track[0].kind == "track"){
					var tracks = _this.parse_tracks(url, _track);
					_track = tracks[0];
				}else{
			...
			//call the ajax
			var req_url = sc_resolve_url+url+
					'&format=json'+
